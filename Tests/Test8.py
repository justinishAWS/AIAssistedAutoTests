"""
Automated end-to-end test for Test #8 (from the APM demo status tracking) using Bedrock Claude 3.7 Sonnet.
...
async def bounding_box(params: PositionParameters, browser: BrowserContext):
    ...
    await page.evaluate(f"""
        ...
        """, args)
    return ActionResult(extracted_content="This function was successfully called. Do not call this again.", include_in_memory=True)
...
async def node_access(browser: BrowserContext):
    page = await browser.get_current_page()
...
task = """
        ...
        2. Open this link.
        3. In the left panel, under Application Signals, click 'Service Map'.
        4. Access the node.
        5. Wait a few seconds.
        6. Click on the bottom link in the right panel.
        7. Click the 'Dependencies' button.
        8. Access the graph and open the popup, PASS in 2 and 3 as a PARAMETERS.
        ...
        13. Close the 'SQS AWS::SQS:Queue' dropdown.
        14. Inside the 'customers-service-java' dropdown, click on the row with 'SQS PurgeQueue' (the last row in this dropdown).
        15. In the right panel, click the right arrow.
        16. In the right panel, click the 'Exceptions' button.
        17. Look for the message 'Only one PurgeQueue operation on apm_test is allowed every 60 seconds.'.
